local util = require "rusty-locale.util"
local prototypes = require "rusty-locale.prototypes"
local recipes = require "rusty-locale.recipes"
...
local function key_of(prototype, type, locale_type)
    ...
    if not locale_type then
        locale_type = prototypes.inherits(prototype.type, _M.localised_types)
    end
...
_M.localised_types = {}
for type, _ in pairs(prototypes.descendants("prototype-base")) do
    _M.localised_types[type] = true
end
...
function _M.of_generic(prototype, locale_type)
    ...
    return util.resolver {
        name = function()
            return prototype.localised_name or key_of(prototype, "name", locale_type)
        end,
        description = function()
            return prototype.localised_description or key_of(prototype, "description", locale_type)
        end
    }
end
...
function _M.of_item(prototype)
    ...
    return util.resolver {
        ...
        name = function(self)
            return prototype.localised_name or self.place_result and self.place_result.name or self.placed_as_equipment_result and self.placed_as_equipment_result.name or key_of(prototype, "name", "item")
        end,
        description = function(self)
            return prototype.localised_description or key_of(prototype, "description", "item")
        end
    }
end
