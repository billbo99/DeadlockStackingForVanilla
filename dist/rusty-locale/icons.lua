...
function _M.of_recipe(prototype)
    ...
    local product
    if prototype.normal ~= nil then
        product = recipes.partial.get_main_product(prototype.normal)
    else
        product = recipes.partial.get_main_product(prototype)
    end
...
function _M.of(prototype, type)
    ...
    if prototypes.inherits(prototype.type, "recipe") then
        return _M.of_recipe(prototype)
    else
        return _M.of_generic(prototype)
    end
end
