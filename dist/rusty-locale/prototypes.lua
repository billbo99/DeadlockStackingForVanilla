local hierarchy = require "rusty-locale.type-hierarchy"
...
function _M.descendants(type)
    ...
    if not is_known(type) then
        log(("Checking descendants of unknown type `%s`!"):format(type))
    end
...
function _M.inherits(t, bases)
    ...
    if type(bases) ~= "table" then
        return _M.inherits(t, {[bases] = true})
    end
    if not is_known(t) then
        log(("Checking inheritance of unknown type `%s`!"):format(t))
    end
...
function _M.find(name, type, silent)
    ...
    for t, prototypes in pairs(data.raw) do
        ...
        if prototype and _M.inherits(t, type) then
            return prototype
        end
    end
    if silent then
    ...
    for t, _ in pairs(_M.find_by_name(name)) do
        table.insert(existing_types, t)
    end
    error(("No prototype called `%s` found for type `%s`, these prototypes with the name exist: %s\nPlease report this to https://mods.factorio.com/mod/rusty-locale"):format(name, type, serpent.line(existing_types)))
end
